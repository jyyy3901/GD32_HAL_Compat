...
static uint32_t TIM_Address(const TIM_HandleTypeDef *htim)
{
    ...
    if ((htim != NULL) && (htim->GD32_RESOURCE != NULL) &&
        (htim->GD32_RESOURCE->stm32_instance == (uintptr_t)htim->Instance))
    {
        return htim->GD32_INSTANCE;
    }
    resource = (htim != NULL) ?
        GD32_HAL_ResolveInstance((uintptr_t)htim->Instance,
                                  ...
                                  void (*msp_init)(TIM_HandleTypeDef *))
{
    ...
    resource = GD32_HAL_ResolveInstance((uintptr_t)htim->Instance,
                                        GD32_HAL_RESOURCE_TIMER);
    if (resource == NULL)
    {
        GD32_HAL_ErrorHook(GD32_HAL_PORT_ERROR_INVALID_INSTANCE,
                           (uint32_t)(uintptr_t)htim->Instance);
        htim->ErrorCode = HAL_TIM_ERROR_INVALID_PARAM;
        return HAL_ERROR;
    }
    htim->GD32_RESOURCE = resource;
    htim->GD32_INSTANCE = resource->gd32_instance;
    htim->GD32_IRQ_NUMBER = resource->gd32_irq;
                                    ...
                                    void (*msp_deinit)(TIM_HandleTypeDef *))
{
    ...
    htim->Lock = HAL_UNLOCKED;
    htim->GD32_INSTANCE = 0U;
    htim->GD32_IRQ_NUMBER = -1;
    htim->GD32_RESOURCE = NULL;
    return HAL_OK;
}
    ...
    const TIM_SlaveConfigTypeDef *sSlaveConfig)
{
    ...
    config.stm32_timer_instance = (uintptr_t)htim->Instance;
