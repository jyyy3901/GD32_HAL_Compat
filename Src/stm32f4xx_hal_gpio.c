...
{
    return (uint32_t)(uintptr_t)GPIOx;
}
...
void HAL_GPIO_Init(GPIO_TypeDef *GPIOx, GPIO_InitTypeDef *GPIO_Init)
{
    ...
    if (((GPIO_Init->Mode & GPIO_MODE) == MODE_AF) && (GPIO_Init->Alternate != 0U))
    {
        /* Alternate 数值不能安全转换为 AFIO remap；拒绝隐式采用错误映射。 */
        GD32_HAL_ErrorHook(GD32_HAL_PORT_ERROR_AF_SELECTION_REQUIRES_NATIVE_INIT,
                           GPIO_Init->Alternate);
        return;
    }
